...
.DESCRIPTION
    ...
    {
      "grand_prize": { // THAY ĐỔI KEY GỐC
        "nominee": [
          {
            // KHÔNG CÒN TRƯỜNG "lucky_number"
            "name": "...",
            "department": "...",
            "about": "...",
            "thumbnail": "thumb/..."
...
.PARAMETER JsonFilePath
    ...
    (Bạn có thể muốn đổi tên file output mặc định, ví dụ: 'grand_prize.json')
.EXAMPLE
    .\Convert-TsvToJson.ps1
    (Sử dụng đường dẫn mặc định cho file TSV và JSON)
.EXAMPLE
    .\Convert-TsvToJson.ps1 -TSVFilePath "C:\data\input_data.tsv" -JsonFilePath "C:\output\grand_prize_data.json"
...
param (
    ...
    # [string]$JsonFilePath = (Join-Path $PSScriptRoot "grand_prize.json")
    [string]$JsonFilePath = (Join-Path $PSScriptRoot "grand_prize.json")
)
...
function Convert-TsvToVotingJson {
    ...
    try {
        ...
        # THAY ĐỔI: Cập nhật danh sách cột bắt buộc (loại bỏ 'lucky_number')
        $requiredColumns = @('name', 'loc', 'self_introduction', 'image')
        ...
        if ($null -ne $tsvData) {
            foreach ($row in $tsvData) {
                ...
                # THAY ĐỔI: Cập nhật đối tượng nominee (loại bỏ lucky_number)
        ...
        $finalJsonObject = [PSCustomObject]@{
            grand_prize = [PSCustomObject]@{ # Key gốc mới
...
if (-not (Test-Path -Path $TSVFilePath -PathType Leaf)) {
    ...
    # THAY ĐỔI: Cập nhật file mẫu (loại bỏ cột 'lucky_number')
    $sampleContent = @"
name`tloc`tself_introduction`timage
Nguyễn Văn A`tPhòng Kỹ Thuật`tỨng cử viên A với nhiều năm kinh nghiệm.`tanh_a.jpg
Trần Thị B`tPhòng Nhân Sự`tỨng cử viên B năng động, sáng tạo.`tanh_b.png
Lê Văn C`tPhòng Marketing`tỨng cử viên C đầy nhiệt huyết.`t
Phạm Thị D`tPhòng Kinh Doanh`tỨng cử viên D.`t    anh_d.gif    
"@
