...
.PARAMETER TSVFilePath
    Đường dẫn đầy đủ đến file TSV đầu vào. Mặc định là 'a.tsv' trong cùng thư mục.
.PARAMETER JsonFilePath
    Đường dẫn đầy đủ để lưu file JSON đầu ra. Mặc định là 'voting_ref.json' trong cùng thư mục.
.EXAMPLE
    .\Convert-TsvToJson.ps1
    (Sử dụng đường dẫn mặc định cho file TSV và JSON)
.EXAMPLE
    .\Convert-TsvToJson.ps1 -TSVFilePath "C:\data\input_data.tsv" -JsonFilePath "C:\output\voting_data.json"
    (Chỉ định đường dẫn tùy chỉnh)
#>
param (
    [string]$TSVFilePath = (Join-Path $PSScriptRoot "data.tsv"),
    [string]$JsonFilePath = (Join-Path $PSScriptRoot "voting_ref.json")
)
...
function Convert-TsvToVotingJson {
    ...
    try {
        ...
        $requiredColumns = @('aggregated', 'name', 'loc', 'self_introduction', 'image')
        ...
        if ($null -ne $tsvData) {
            foreach ($row in $tsvData) {
                ...
                $nomineeObject = [PSCustomObject]@{
                    aggregate         = ($row.aggregated | Get-Member -Name Trim -MemberType Method -ErrorAction SilentlyContinue) ? $row.aggregated.Trim() : $row.aggregated
...
if (-not (Test-Path -Path $TSVFilePath -PathType Leaf)) {
    ...
    # Cập nhật file mẫu để có cột 'aggregated'
    $sampleContent = @"
aggregated`tname`tloc`tself_introduction`timage
Người bình chọn 1`tNguyễn Văn A`tPhòng Kỹ Thuật`tỨng cử viên A với nhiều năm kinh nghiệm.`tanh_a.jpg
Người bình chọn 2`tTrần Thị B`tPhòng Nhân Sự`tỨng cử viên B năng động, sáng tạo.`tanh_b.png
Người bình chọn 3`tLê Văn C`tPhòng Marketing`tỨng cử viên C đầy nhiệt huyết.`t
Người bình chọn 4`tPhạm Thị D`tPhòng Kinh Doanh`tỨng cử viên D.`t    anh_d.gif    
"@
    Set-Content -Path $TSVFilePath -Value $sampleContent -Encoding UTF8
    Write-Host "Đã tạo file mẫu '$TSVFilePath' với encoding UTF-8 và cột 'aggregated'."
}
